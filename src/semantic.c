...
static expty transExp(Tr_level level, S_table venv, S_table tenv, A_exp e) {
  ...
  case A_opExp: {
    ...
    case A_neqOp:
      ...
      default: {
        EM_error(e->u.op.right->pos, "unexpected %s expression in comparsion",
                 Ty_ToString(right.ty));
      }
      }
  ...
  case A_ifExp: {
    ...
    if (test.ty->kind != Ty_int && test.ty->kind != Ty_bool) {
      EM_error(e->pos,
               "test block must be integer or bool in condition expression");
      exit(1);
    }
    ...
    if (e->u.iff.elsee) {
      ...
      if (then.ty->kind != Ty_void) {
        EM_error(e->pos, "then block must be void in condition expression");
        exit(1);
      }
  ...
  case A_breakExp: {
    if (!inside) {
      EM_error(e->pos, "break outside loop in break expression");
      exit(1);
    }
  ...
  case A_varDec: {
    ...
    if (dec_ty != NULL) {
      if (!actual_eq(dec_ty, init_exp.ty)) {
        EM_error(
            d->pos,
            "dismatch type between declare and initialze in variable declare");
        exit(1);
      }
    } else {
    ...
    E_enventry eentry = E_VarEntry(m_access, init_exp.ty);
