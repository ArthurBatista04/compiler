...
<INSTRING>\n  {adjust(); EM_error(EM_tokPos,"unterminated string: string cannot be declared in two separated lines"); yyterminate();}
<INSTRING><<EOF>> {adjust(); EM_error(EM_tokPos,"unterminated string"); yyterminate();}
<INSTRING>\\[0-9]{3} {adjust(); int tmp; sscanf(yytext+1, "%d", &tmp);
                      if(tmp > 0xff) { EM_error(EM_tokPos,"invalid characters"); yyterminate(); }
                      append_to_buffer(tmp);
                      }
<INSTRING>\\[0-9]+ {adjust(); EM_error(EM_tokPos,"error in escape sequence"); yyterminate();}
...
<COMMENT><<EOF>> {adjust(); EM_error(EM_tokPos,"unterminated comment"); yyterminate();}
<COMMENT>.    {adjust();}

  /* unknown input */
<INITIAL>.	 {adjust(); EM_error(EM_tokPos,"token not recognized");}
