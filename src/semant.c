...
static int ty_equal(Ty_ty t1, Ty_ty t2) {
  ...
  if (at1->kind == Ty_record && at2->kind == Ty_nil)
    return 1;
  else if (at1->kind == Ty_nil && at2->kind == Ty_record)
    return 1;
  else
    return at1 == at2;
}
...
struct expty transVar(Tr_level level, S_table venv, S_table tenv, A_var v,
    ...
    if ((var.ty)->kind != Ty_record) {
      ...
      for (; tmp; tmp = tmp->tail) {
        if (tmp->head->name == (v->u.field.sym)) {
          return expTy(Tr_fieldVar(var.exp, Tr_intExp(i)),
                       actual_ty(tmp->head->ty));
        }
        i++;
      }
  ...
  case A_subscriptVar: {
    ...
    return expTy(Tr_subsriptVar(var.exp, exp.exp),
...
struct expty transExp(Tr_level level, S_table venv, S_table tenv, A_exp a,
  ...
  case A_boolExp: {
    return expTy(Tr_boolExp(a->u.intt), Ty_Bool());
  }
  case A_stringExp: {
  ...
  case A_callExp: {
    ...
    for (; pargs && ats; pargs = pargs->tail, ats = ats->tail) {
      if (actual_ty(pargs->head)->kind == Ty_record &&
          actual_ty(ats->head)->kind == Ty_nil)
        continue;
      if (actual_ty(pargs->head) != actual_ty(ats->head)) {
        EM_error(a->pos, "parameter mismach");
      }
    }
    return expTy(Tr_callExp(func->u.fun.label, aes, level, func->u.fun.level),
  ...
  case A_opExp: {
    ...
    if (actual_ty(left.ty)->kind == Ty_record &&
        ...
        actual_ty(right.ty)->kind == Ty_record &&
  ...
  case A_recordExp: {
    ...
    for (; tmp; tmp = tmp->tail)
      record_count++;
    int *offset =
        checked_malloc(sizeof(int) * record_count); //��¼��Ӧ����λ�õ�ƫ��
    //�����������������Ƿ��Ӧ����
    ...
    for (; tl; tl = tl->tail, k++) {
      ...
      for (; tmp; tmp = tmp->tail, i++) {
        if (name == tmp->head->name &&
            (actual_ty(e.ty) == actual_ty(tmp->head->ty) ||
             actual_ty(e.ty)->kind == Ty_nil &&
                 actual_ty(tmp->head->ty)->kind == Ty_record)) {
          flag = 1;
          offset[k] = i;
          break;
        }
  ...
  case A_forExp: {
    ...
    S_endScope(venv);
    Tr_exp fff =
        Tr_forExp(Tr_simpleVar(i, level), e1.exp, e2.exp, e3.exp, breakk);
    breakk = oldbreak;
...
Tr_exp transDec(Tr_level level, S_table venv, S_table tenv, A_dec d,
  ...
  case A_functionDec: {
    ...
    for (; funcs; funcs = funcs->tail) {
      ...
      Tr_level newlevel =
          Tr_newLevel(level, Temp_namedlabel(S_name(f->name)), formalBools);
      if (levell == NULL) {
      ...
      S_enter(
          venv, f->name,
          E_FunEntry(newlevel, Tr_levelName(newlevel), formalTys, resultTy));
    }
    funcs = d->u.function;
    pp = levell;
    for (; funcs; funcs = funcs->tail, pp = pp->tail) {
      ...
      S_beginScope(venv);
      {
        ...
        Ty_tyList t;
        Tr_accessList acc;
        for (l = f->params, t = formalTys, acc = accss; l;
