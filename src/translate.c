...
Temp_label Tr_levelName(Tr_level level) { return F_name(level->frame); }
...
Tr_exp Tr_iffExp(Tr_exp test, Tr_exp then, Tr_exp elsess) {
  if (elsess)
    return Tr_ifthenelse(test, then, elsess);
  else
    return Tr_ifthen(test, then);
}

Tr_exp Tr_recordExp(Tr_expList exps, int offsets[], int n) {
  Temp_temp r = Temp_newtemp();
  T_exp mmalloc = F_externalCall(String("allocRecord"),
                                 T_ExpList(T_Const(n * get_wordsize()), NULL));
  T_stm mov = T_Move(T_Temp(r), mmalloc);
  int i = 0;
  for (; exps; exps = exps->tail, i++) {
    mov =
        T_Seq(mov, T_Move(T_Mem(T_Binop(T_plus, T_Temp(r),
                                        T_Const(offsets[i] * get_wordsize()))),
                          unEx(exps->head)));
  }
...
Tr_exp Tr_breakExp(Tr_exp breakk) {
  return Tr_Nx(
      T_Jump(unEx(breakk), Temp_LabelList(unEx(breakk)->u.NAME, NULL)));
}

Tr_exp Tr_doneExp(void) { return Tr_Ex(T_Name(Temp_newlabel())); }
...
Tr_exp Tr_whileExp(Tr_exp test, Tr_exp body, Tr_exp breakk) {
  ...
  return Tr_Nx(T_Seq(
      T_Label(loop),
      T_Seq(e1.stm, T_Seq(T_Label(t), T_Seq(e2, T_Seq(jump, T_Label(done)))))));
}
          ...
          T_Seq(lo_le_hi,
                T_Seq(T_Label(into),
                      T_Seq(unNx(body),
                            T_Seq(test,
                                  T_Seq(T_Label(next),
                                        T_Seq(inc, T_Seq(jump,
                                                         T_Label(end)))))))))));
}

Tr_exp Tr_callExp(Temp_label name, Tr_expList oargs, Tr_level calllevel,
                  Tr_level funlevel) {
  if (funlevel != NULL) {
...
Tr_exp Tr_nopExp(void) { return Tr_Ex(T_Const(0)); }

Tr_exp Tr_nilExp(void) { return Tr_Ex(T_Const(0)); }

Tr_exp Tr_intExp(int v) { return Tr_Ex(T_Const(v)); }
Tr_exp Tr_boolExp(int v) { return Tr_Ex(T_Const(v)); }
...
void Tr_setargnum(Tr_level level, int n) { F_set_maxarg(level->frame, n + 1); }
