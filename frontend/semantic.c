...
static expty transExp(Tr_level level, S_table venv, S_table tenv, A_exp e) {
  ...
  case A_boolExp:
    return expTy(Tr_boolExp(e->u.intt), Ty_Bool());
  case A_stringExp:
  ...
  case A_ifExp: {
    ...
    if (test.ty->kind != Ty_int && test.ty->kind != Ty_bool) {
      EM_error(e->pos,
               "condition expression: test section must be integer or bool");
      exit(1);
    }
  ...
  case A_whileExp: {
    ...
    if (test.ty->kind != Ty_int && test.ty->kind != Ty_bool) {
      EM_error(e->u.whilee.test->pos,
               "while loop: test section must produce integer or bool");
      exit(1);
    }
    inside++; // inside loop
...
static Tr_exp transDec(Tr_level level, S_table venv, S_table tenv, A_dec d) {
  ...
  case A_varDec: {
    ...
    if (dec_ty != NULL) {
      ...
      if (init_exp.ty->kind == Ty_nil) {
        EM_error(d->pos, "variable declare: illegal nil type: nil must be "
                         "assign to a explictly record type");
        exit(1);
      }
    }
  ...
  case A_typeDec: {
      ...
      for (int i = 0; i < index; i++) {
        if (typenames[i] == (void *)type_list->head->name) {
          EM_error(type_list->head->ty->pos,
                   "type declare: redeclaration type <%s>, there are two types "
                   "with the same name in the same (consecutive) batch of "
    ...
    if (cycle_decl) {
      EM_error(d->pos, "type declare: illegal cycle type declaration: must "
                       "contain at least one built-in type");
      exit(1);
    }
    return Tr_noExp();
  }
  case A_functionDec: {
    ...
    for (fun_list = d->u.function; fun_list; fun_list = fun_list->tail) {
      ...
      for (fl = fun_list->head->params; fl; fl = fl->tail) {
        ...
        if (m_head) {
          m_tail->tail =
              U_BoolList(1, NULL); // to keep things easy, vars are escaping
          m_tail = m_tail->tail;
        } else {
